...
def get_lido_validators_metrics(collection: Collection, period: str, calc_ratio: bool) -> List[dict]:
    ...
    if period == "last_week" and calc_ratio:
        data = collection.find_one({"metrics": "last_week_validators_censorship_resistance_index"})
        ...
        data = collection.find_one({"metrics": "last_month_validators_censorship_resistance_index"})
    elif period == "last_month" and not calc_ratio:
        data = collection.find_one({"metrics": "last_month_validators_compliance_ratio"})
    else:
...
def get_lido_vs_rest(collection: Collection, period: str) -> List[dict]:
    ...
    """
    if period == "last_week":
        data = collection.find_one({"metrics": "last_week_lido_vs_rest_censorship_resistance_index"})
    elif period == "last_month":
        data = collection.find_one({"metrics": "last_month_lido_vs_rest_censorship_resistance_index"})
    else:
        raise ValueError("Wrong period")
...
def get_censored_latency(collection: Collection) -> List[dict]:
    """
    Calculate the censorship latency for
    non OFAC compliant censored transactions
