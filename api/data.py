...
def _get_daterange(start_date: str, end_date: str) -> List[str]:
    ...
    daterange = [str_date_repr(start_date + datetime.timedelta(days=diff)) for diff in range(daydiff)]
...
def get_metrics_by_validators_by_daterange(collection: Collection, names: List[str], start_date: str, end_date: str) -> List[dict]:
    daterange = _get_daterange(start_date, end_date)
    keys = get_collection_keys(collection)
