...
def update_metrics(
    ...
    update_censored_percentage(txs_collection, validators_collection, target_collection, logger)
    update_overall_average_latency(txs_collection, validators_collection, target_collection, logger)
    update_censored_average_latency(txs_collection, validators_collection, target_collection, logger)
    update_lido_vs_rest_censorship_resistance_index(metrics_collection, target_collection, logger)
    update_validators_censorship_resistance_index(metrics_collection, target_collection, logger)
    update_validators_compliance_ratio(metrics_collection, target_collection, logger)
...
def update_censored_percentage(
    ...
    percentage = get_censored_percentage(txs_collection, validators_collection, "last_month")
...
def update_censored_average_latency(
    ...
    latency = get_censored_latency(txs_collection, validators_collection, "average")
    record = {"metrics": "censored_average_latency", "values": latency}

    logger.info("Average latency for censored transactions has been calculated")
    ...
    logger.info("Average latency for censored transactions has been updated")
...
def update_lido_vs_rest_censorship_resistance_index(
    ...
    logger.info("Calculating Lido vs Rest censorship resistance index over last week")

    lw_resistance_index = get_lido_vs_rest(metrics_collection, "last_week")

    logger.info("Lido vs Rest censorship resistance index over last week has been calculated")
    logger.info("Calculating Lido vs Rest censorship resistance index over last month")

    lm_resistance_index = get_lido_vs_rest(metrics_collection, "last_month")

    logger.info("Lido vs Rest censorship resistance index over last month has been calculated")
    ...
    logger.info("Updating Lido vs Rest censorship resistance index over last week")

    target_collection.delete_one({"metrics": "last_week_lido_vs_rest_censorship_resistance_index"})
    target_collection.insert_one(lw_record)

    logger.info("Lido vs Rest censorship resistance index over last week has been updated")
    logger.info("Updating Lido vs Rest censorship resistance index over last month")

    target_collection.delete_one({"metrics": "last_month_lido_vs_rest_censorship_resistance_index"})
    target_collection.insert_one(lm_record)

    logger.info("Lido vs Rest censorship resistance index over last month has been updated")
...
def update_validators_censorship_resistance_index(
    ...
    logger.info("Calculating validators censorship resistance index over last week")

    lw_resistance_index = get_lido_validators_metrics(metrics_collection, "last_week", True)

    logger.info("Validators censorship resistance index over last week has been calculated")
    logger.info("Calculating validators censorship resistance index over last month")

    lm_resistance_index = get_lido_validators_metrics(metrics_collection, "last_month", True)

    logger.info("Validators censorship resistance index over last month has been calculated")
    ...
    logger.info("Updating validators censorship resistance index over last week")

    target_collection.delete_one({"metrics": "last_week_validators_censorship_resistance_index"})
    target_collection.insert_one(lw_record)

    logger.info("Validators censorship resistance index over last week has been updated")
    logger.info("Updating validators censorship resistance index over last month")

    target_collection.delete_one({"metrics": "last_month_validators_censorship_resistance_index"})
    target_collection.insert_one(lm_record)

    logger.info("Validators censorship resistance index over last month has been updated")
...
def update_validators_compliance_ratio(
    ...
    lw_ratio = get_lido_validators_metrics(metrics_collection, "last_week", False)

    logger.info("Validators compliance ratio over last week has been calculated")
    logger.info("Calculating validators compliance ratio over last month")

    lm_ratio = get_lido_validators_metrics(metrics_collection, "last_month", False)

    logger.info("Validators compliance ratio over last month has been calculated")
    ...
    target_collection.delete_one({"metrics": "last_week_validators_compliance_ratio"})
    target_collection.insert_one(lw_record)

    logger.info("Validators compliance ratio over last week has been updated")
    logger.info("Updating validators compliance ratio over last month")

    target_collection.delete_one({"metrics": "last_month_validators_compliance_ratio"})
    target_collection.insert_one(lm_record)

    logger.info("Validators compliance ratio index over last month has been updated")


def _get_validators_metrics(collection: Collection, dates: List[str]) -> pd.DataFrame:
    """
    Create dataframe from mongo metrics collection's records
    corresponding to the specific dates
    ...
    try:
        ...
        raise OperationFailure("Failed to fetch metrics data from db")
...
def _get_ofac_compliant_count(row: pd.Series, dates: List[str]) -> int:
    """
    Count OFAC compliant transactions over specific dates
    for the specific validator

    Args:
        row     -   Pandas series corresconding to the specific validator
        dates   -   List of dates for which transaction's count
                    need to be calculated

    Returns:
        Count of OFAC compliant transactions over specific dates
        for the specific validator
    """
    count = 0
    for day in dates:
...
def _get_ofac_non_compliant_count(row: pd.Series, dates: List[str]) -> int:
    """
    Count OFAC non compliant transactions over specific dates
    for the specific validator
    ...
    Returns:
        Count of OFAC non compliant transactions over specific dates
        for the specific validator
    """
    count = 0
    for day in dates:
...
def _get_ofac_compliant_share(row: pd.Series, df: pd.DataFrame) -> float:
    """
    Calculate the share of a particular validator in the validation
    of ofac compliant transactions inside Ethereum blockchain
    ...
    Returns:
        Share of a particular validator in the validation of
        ofac compliant transactions inside Ethereum blockchain in percents
    """
...
def _get_ofac_non_compliant_share(row: pd.Series, df: pd.DataFrame) -> float:
    """
    Calculate the share of a particular validator in the validation
    of ofac non compliant transactions inside Ethereum blockchain
    ...
    Returns:
        Share of a particular validator in the validation of
        ofac non compliant transactions inside Ethereum blockchain in percents
    """
...
def _get_ratio(row: pd.Series) -> float:
    """
    Calculate the ratio between validator's ofac non compliant
    share and validator's ofac compliant share

    Args:
        row -   Pandas series corresconding to the specific validator

    Returns:
        Ratio between validator's ofac non compliant share
        and validator's ofac compliant share
    """
    ratio = 1

    if row["ofac_compliant_share"] != 0:
        ratio = row["ofac_non_compliant_share"] / row["ofac_compliant_share"]
    else:
        pass
...
def _calc_lido_latency(censored_blocks: List[dict], lido_vals: List[str]) -> int:
    """
    Calculate the censorship latency in case the Lido validators
    would not censor OFAC non compliant transactions
    ...
    Returns:
        Censorship latency in case the Lido validators
        would not censor OFAC non compliant transactions in seconds
    """
    latency = 0
...
def _get_count_of_censored_blocks(censored: List[dict] | Any) -> int:
    """
    ...
    if isinstance(censored, list):
        censored_blocks = len(censored)
    else:
        censored_blocks = 0

    return censored_blocks


def get_lido_validators_metrics(collection: Collection, period: str, calc_ratio: bool) -> List[dict]:
    """
    Calculate censorship metrics over specific time period

    Args:
        collection  -   Mongo collection of metrics
        period      -   Time period for which censorship metrics
                        need to be calculated (last_week or last_month)
...
def get_lido_vs_rest(collection: Collection, period: str) -> List[dict]:
    ...
    other_ofac_compliant_share = (
        metrics_df[metrics_df.name == "Other"].ofac_compliant_count.values[0] / total_ofac_compliant_count
    )
    other_ofac_non_compliant_share = (
        metrics_df[metrics_df.name == "Other"].ofac_non_compliant_count.values[0] / total_ofac_non_compliant_count
    )
...
def get_overall_latency(txs_collection: Collection, validators_collection: Collection) -> List[dict]:
    """
    ...
    try:
        ...
        raise OperationFailure("Failed to fetch validators data from db")

    for shift in range(week_diff + 1):
        # For each available week find it's boundaries
        monday_ts, sunday_ts, monday_dt, sunday_dt = get_shifted_week(first_monday, first_sunday, shift)
        ...
        shifted_df["censorship_latency"] = shifted_df.censored.apply(_get_count_of_censored_blocks) * 12
        shifted_df["censorship_latency_without_lido_censorship"] = shifted_df.censored.apply(
        ...
        record = {
            ...
            "overall_censorship_latency_without_lido_censorship": shifted_df.\
...
def get_censored_latency(
    ...
    """
    Calculate the censorship latency for non OFAC
    compliant censored transactions
    ...
    for shift in range(week_diff + 1):
        ...
        if mean == "average":
            record = {
                ...
                "average_censorship_latency_without_lido_censorship": shifted_df\
                    .censorship_latency_without_lido_censorship\
                    .mean(),
            }
        if mean == "median":
            record = {
                ...
                "median_censorship_latency_without_lido_censorship": shifted_df\
                    .censorship_latency_without_lido_censorship\
                    .median(),
            }
...
def get_censored_percentage(
    ...
    censored_txs_df["censorship_latency_without_lido_censorship"] = censored_txs_df.censored.apply(
        _calc_lido_latency, args=(lido_vals,)
    )

    lido_censored_txs_df = censored_txs_df[
        censored_txs_df.censorship_latency != censored_txs_df.censorship_latency_without_lido_censorship
    ]

    percentage = {
        "censored_percentage": len(censored_txs_df) / len(non_compliant_txs_df) * 100,
        "lido_censored_percentage": len(lido_censored_txs_df) / len(non_compliant_txs_df) * 100,
    }
